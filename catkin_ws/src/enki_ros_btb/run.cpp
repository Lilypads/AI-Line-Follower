...
public:
	...
	camera_pub2 = nh.advertise<sensor_msgs::Image>("mybot/camera2/image_raw", 1);
	...
	food = new PhysicalObject();
	food->setCylindric(5,3,1);	//radius, height, mass
	food->setColor(Enki::Color(1,0,0));
	food->pos = Point(maxx/4, maxy*3/4);
	world->addObject(food);

        //set camera sensor pixels -> 9x9 array of ground sensors infront of robotP
...
virtual void sceneCompletedHook()
	...
	msg.height = 9;
        msg.width = 9;
        msg.step = 9*4; //need to equal width*4(rgba) + add*2 if rgba16
...
int main(int argc, char *argv[])
{
   ...
   //Creating combpath which directs to our texture file location
	std::string pckpath = ros::package::getPath("enki_ros_btb"); //C++ string path to enki_ros_pck package folder
    ...
    ros::init(argc,argv,"enki_ros_btb_node");
